...
float ComputePathGCost( Map& theMap, const TileCoordsList& path )
{
	...
	for( int stepIndex = 0; stepIndex < (int) path.size(); ++ stepIndex )
	{
		...
		float gCostHere = MapTile::GetGCostForTileType( tileType );
		totalCost += gCostHere;
	}
...
//	srand( (unsigned int) time(nullptr) );
	...
	Map theMap( 70, 20 ); // 13x9 for debugging, 70x20 for small map test, 180x32 medium, 180x180 large, 180x900 huge

	TestPathingCorrectness( theMap, 100, 3.f );
	TestPathingPerformance( theMap, 10000, 3.f );
