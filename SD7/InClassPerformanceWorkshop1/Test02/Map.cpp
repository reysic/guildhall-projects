	...
	, m_height( height )
{
	...
	for( int tileIndex = 0; tileIndex < numTiles; ++ tileIndex )
	{
		...
		m_gCosts.push_back( GetGCostForTileType( TILE_TYPE_UNKNOWN ) );
		m_tileGlyphs.push_back( GetGlyphForTileType( TILE_TYPE_UNKNOWN ) );
	}
...
void Map::ChangeTileType( int tileIndex, TileType newType )
{
	...
	m_gCosts[ tileIndex ] = GetGCostForTileType( newType );
	m_tileGlyphs[ tileIndex ] = GetGlyphForTileType( newType );
}
...
bool Map::IsTileSolid( const IntVector2& tileCoords ) const
{
	...
	return IsTileTypeSolid( tileType );
}
...
{
	int tileIndex = GetTileIndexForCoords( tileX, tileY );
	char tileGlyph = m_tileGlyphs[ tileIndex ];
	return tileGlyph;
}
...
{
	TileType tileType = GetTileTypeAtCoords( tileCoords );
	return GetGCostForTileType( tileType );
}
...
void Map::OpenNeighborAsAppropriate( const IntVector2& neighborCoords, PathNode* parent, PathNodeList& openNodes, PathNodeList& closedNodes, const IntVector2& goalCoords ) const
{
	...
	if( IsTileTypeSolid( neighborType ) )
	{
	...
	float gCostForNeighbor = GetGCostForTileType( neighborType );
...
//-----------------------------------------------------------------------------------------------
float Map::GetGCostForTileType( TileType tileType ) const
{
	switch ( tileType )
	{
...
//-----------------------------------------------------------------------------------------------
unsigned char Map::GetGlyphForTileType( TileType tileType )
{
	switch ( tileType )
	{
...
//-----------------------------------------------------------------------------------------------
bool Map::IsTileTypeSolid( TileType tileType ) const
{
	switch ( tileType )
	{
...
//-----------------------------------------------------------------------------------------------
std::string Map::GetNameForTileType( TileType tileType )
{
	switch ( tileType )
	{
